...
export default function HeroCarousel() {
  ...
  const showPrevious = useCallback(() => changeImage((activeIndex - 1 + images.length) % images.length), [activeIndex, changeImage]);
  ...
  return <div className="relative aspect-[4/3] overflow-hidden rounded-3xl" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
    ...
    <button type="button" onClick={showPrevious} aria-label="Imagem anterior" className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-marrom-escuro shadow-sm transition-colors hover:bg-white focus-visible:outline-none"><ChevronLeft size={22} aria-hidden="true" /></button>
    <button type="button" onClick={showNext} aria-label="Próxima imagem" className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-marrom-escuro shadow-sm transition-colors hover:bg-white focus-visible:outline-none"><ChevronRight size={22} aria-hidden="true" /></button>
